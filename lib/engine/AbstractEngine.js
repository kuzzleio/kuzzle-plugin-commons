...
class AbstractEngine {
    ...
    async create(index, group = 'commons', request) {
        ...
        const { collections } = await this.onCreate(index, group, request);
    ...
    async update(index, group = 'commons', request) {
        if (!await this.exists(index)) {
        ...
        await this.engineConfigManager.createCollection(index);
        const { collections } = await this.onUpdate(index, group, request);
        return { collections };
    }
    async delete(index, request) {
        if (!await this.exists(index)) {
            throw new kuzzle_1.NotFoundError(`${kuzzle_1.Inflector.upFirst(this.pluginName)} engine on index "${index}" does not exists`);
        }
