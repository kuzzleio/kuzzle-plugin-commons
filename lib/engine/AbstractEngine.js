...
class AbstractEngine {
     ...
     */
    constructor(pluginName, plugin, adminIndex, adminConfigCollection) {
    ...
    async create(index, group = 'commons') {
        ...
        await this.sdk.document.create(this.adminIndex, this.adminConfigCollection, { type: this.configType, engine: { index, group } }, this.engineId(index), { refresh: 'wait_for' });
        return { collections };
    }
    ...
    async delete(index) {
        ...
        try {
            ...
            await this.sdk.document.delete(this.adminIndex, this.adminConfigCollection, this.engineId(index), { refresh: 'wait_for' });
        }
    }
    async list() {
        const result = await this.sdk.document.search(this.adminIndex, this.adminConfigCollection, {
            query: {
    ...
    async exists(index) {
        const exists = await this.sdk.document.exists(this.adminIndex, this.adminConfigCollection, this.engineId(index));
        return exists;
    }
    /**
