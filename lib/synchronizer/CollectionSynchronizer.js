...
class CollectionSynchronizer {
    constructor(app, srcCollection, dstCollection) {
        ...
        this.app.pipe.register('generic:document:afterWrite', (documents, request) => this.pipeAfterWrite(documents, request));
        this.app.pipe.register('generic:document:afterUpdate', (documents, request) => this.pipeAfterUpdate(documents, request));
        this.app.pipe.register('generic:document:beforeDelete', (documents, request) => this.pipeBeforeDelete(documents, request));
    }
    start() {
        this.enabled = true;
    }
    ...
    async pipeBeforeDelete(documents, request) {
        ...
        if (filteredDocs.length > 0) {
            ...
            await this.app.sdk.document.mDelete(request.getIndex(), this.dstCollection, ids);
